-- The appointment RPC needs to write both appointments and work_items atomically.
-- It performs explicit tenant and role checks before bypassing table RLS.
...
begin
  if caller_id is null or not exists (
    ...
    where membership.organization_id = target_organization_id
      ...
      and membership.role in ('owner', 'admin', 'reception')
  ) then
  ...
  insert into public.appointments (
    ...
    target_starts_at, target_starts_at + make_interval(mins => service_duration), target_notes, caller_id
