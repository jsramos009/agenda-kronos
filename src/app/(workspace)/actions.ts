...
export async function createService(
  ...
  if (!parsed.success)
    return {
      status: "error",
      message: "Revise nome, duração e valor do serviço.",
    };
  ...
  try {
    ...
    return {
      status: "error",
      message:
        error instanceof Error ? error.message : "Falha ao criar serviço.",
    };
  }
}
...
export async function createAppointment(
  ...
  if (
    !parsed.success ||
    (!parsed.data.title && !parsed.data.customerId && !parsed.data.serviceId)
  )
    return {
      status: "error",
      message: "Informe um título, cliente ou serviço e revise os horários.",
    };
  try {
    const { workspace, supabase } = await tenantContext();
    const { data: claimData } = await supabase.auth.getClaims();
    const userId = claimData?.claims?.sub;
    const [{ data: member }, { data: stage }, { data: organization }] =
      await Promise.all([
        supabase
    ...
    if (endsAt <= startsAt)
      return { status: "error", message: "Escolha início e fim válidos." };
...
export async function rescheduleAppointment(
  ...
  if (!parsed.success)
    return {
      status: "error",
      message: "Escolha uma data e um horário válidos.",
    };
  ...
  try {
    ...
    const { data: previousAppointment } = await supabase
      ...
      .eq("organization_id", workspace.organizationId)
      .maybeSingle();
    if (!previousAppointment?.starts_at || !previousAppointment.ends_at)
      return { status: "error", message: "Agendamento não encontrado." };
    const duration =
      new Date(previousAppointment.ends_at).getTime() -
      new Date(previousAppointment.starts_at).getTime();
    const { error } = await supabase.rpc("update_flexible_appointment_time", {
      ...
      target_ends_at: new Date(
        parsed.data.startsAt.getTime() + duration,
      ).toISOString(),
      target_action: "appointment.rescheduled",
    });
    if (error?.code === "23P01")
      return { status: "error", message: "Esse horário já está ocupado." };
    if (error) throw error;
    revalidatePath("/agenda");
    revalidatePath("/dashboard");
    revalidatePath("/relatorios");
...
export async function updateAppointmentNotes(
  ...
  const parsed = z
    ...
    .safeParse({ appointmentId, notes });
  if (!parsed.success)
    return {
      status: "error",
      message: "As observações devem ter até 1.000 caracteres.",
    };
  try {
    const { workspace, supabase } = await tenantContext();
...
export async function setMemberAccess(
  ...
  if (!z.string().uuid().safeParse(memberId).success)
    return { status: "error", message: "Pessoa inválida." };
  try {
    const { workspace, supabase } = await tenantContext();
    ...
    if (error?.code === "42501")
      return {
        status: "error",
        message:
          "O proprietário e o seu próprio acesso não podem ser suspensos.",
      };
    if (error) throw error;
    ...
    return {
      status: "success",
      message: active ? "Acesso reativado." : "Acesso suspenso.",
    };
  } catch (error) {
    return {
      status: "error",
      message:
        error instanceof Error ? error.message : "Falha ao alterar o acesso.",
    };
  }
}
...
export async function moveWorkItem(
  ...
  if (
    !z.string().uuid().safeParse(workItemId).success ||
  ...
  try {
    ...
    return {
      status: "error",
      message:
        error instanceof Error ? error.message : "Falha ao mover atendimento.",
    };
  }
}
...
export async function inviteTeamMember(
  ...
  const parsed = z
    .object({
      ...
      role: z.enum(["admin", "reception", "professional", "analyst"]),
    })
    .safeParse({ email: formData.get("email"), role: formData.get("role") });
  if (!parsed.success)
    return {
      status: "error",
      message: "Informe um e-mail e um papel válidos.",
    };
  try {
    const { workspace, supabase } = await tenantContext();
    ...
    return {
      status: "error",
      message:
        error instanceof Error ? error.message : "Falha ao registrar convite.",
    };
  }
}
...
export async function saveOrganizationSettings(
  ...
  const parsed = z
    .object({
      ...
      slotIntervalMinutes: z.coerce
        .number()
        .int()
        .refine((value) => [10, 15, 30, 60].includes(value)),
      selectedServices: z
  ...
  if (!parsed.success)
    return {
      status: "error",
      message: "Revise os dados, horários e cores antes de salvar.",
    };
  const allowedServices = new Set(
    niches[parsed.data.nicheId].services.map((service) => service.name),
  );
  if (
    !parsed.data.selectedServices.every((service) =>
      allowedServices.has(service),
    )
  ) {
    return {
      status: "error",
      message: "Selecione apenas modelos disponíveis para o nicho atual.",
    };
  }
  ...
  if (
    ...
    logo.size > 0 &&
    (!logoExtensions[logo.type] || logo.size > 5 * 1024 * 1024)
  ) {
    return {
      status: "error",
      message: "Use um logo PNG, JPG ou WebP de até 5 MB.",
    };
  }
  try {
    const { workspace, supabase } = await tenantContext();
    const preferences = {
      agenda: {
        startsAt: parsed.data.agendaStart,
        endsAt: parsed.data.agendaEnd,
        days: parsed.data.agendaDays,
    ...
    const serviceTemplates = niches[parsed.data.nicheId].services.filter(
      (service) => parsed.data.selectedServices.includes(service.name),
    );
    const { error: disableServicesError } = await supabase
      .from("services")
      .update({ active: false })
      .eq("organization_id", workspace.organizationId);
    ...
    const stageUpdates = (stages ?? [])
      .slice(0, parsed.data.workflowNames.length)
      .map((stage, index) =>
        supabase
          .from("workflow_stages")
          .update({ name: parsed.data.workflowNames[index] })
          .eq("id", stage.id)
          .eq("organization_id", workspace.organizationId),
      );
    ...
    const { data: availabilityRules, error: availabilityReadError } =
      await supabase
        .from("availability_rules")
        .select("id, weekday, member_id, resource_id")
        .eq("organization_id", workspace.organizationId);
    if (availabilityReadError) throw availabilityReadError;
    const selectedDays = new Set(parsed.data.agendaDays);
    const availabilityUpdates = (availabilityRules ?? []).map((rule) =>
      supabase
        .from("availability_rules")
        .update({
          starts_at: parsed.data.agendaStart,
          ends_at: parsed.data.agendaEnd,
          active: selectedDays.has(rule.weekday),
        })
        .eq("id", rule.id)
        .eq("organization_id", workspace.organizationId),
    );
    const availabilityUpdateResults = await Promise.all(availabilityUpdates);
    const availabilityUpdateError = availabilityUpdateResults.find(
      (result) => result.error,
    )?.error;
    if (availabilityUpdateError) throw availabilityUpdateError;
    const existingDays = new Set(
      (availabilityRules ?? []).map((rule) => rule.weekday),
    );
    const missingDays = parsed.data.agendaDays.filter(
...
export async function updateFlexibleAppointment(
  _: ActionState,
  formData: FormData,
): Promise<ActionState> {
  ...
  if (!parsed.success)
    return {
      status: "error",
      message: "Revise os dados do evento antes de salvar.",
    };
  try {
    const { workspace, supabase } = await tenantContext();
    ...
    if (error?.code === "23P01")
      return {
        status: "error",
        message: "O novo período conflita com outro evento.",
      };
    if (error) throw error;
    revalidatePath("/agenda");
    revalidatePath("/dashboard");
    ...
    return {
      status: "error",
      message:
        error instanceof Error ? error.message : "Falha ao atualizar evento.",
    };
  }
}
...
export async function resizeAppointment(
  ...
  if (!parsed.success)
    return { status: "error", message: "Escolha uma duração válida." };
  ...
  try {
    ...
    if (!appointment?.starts_at)
      return {
        status: "error",
        message: "Agendamento não encontrado neste espaço.",
      };
    ...
    const { error } = await supabase.rpc("update_flexible_appointment_time", {
      target_organization_id: workspace.organizationId,
      target_appointment_id: parsed.data.appointmentId,
      target_starts_at: appointment.starts_at,
      target_ends_at: endsAt.toISOString(),
      target_action: "appointment.resized",
    });
    if (error?.code === "23P01")
      return {
        status: "error",
        message: "A nova duração invade outro atendimento.",
      };
    ...
    return {
      status: "error",
      message:
        error instanceof Error ? error.message : "Falha ao alterar a duração.",
    };
  }
}
...
export async function updateRecommendation(
  ...
  if (!z.string().uuid().safeParse(recommendationId).success)
    return { status: "error", message: "Recomendação inválida." };
  try {
    const { workspace, supabase } = await tenantContext();
    const values =
      status === "applied"
        ? { status, applied_at: new Date().toISOString(), snoozed_until: null }
        : status === "snoozed"
          ? {
              status,
          ...
          : { status, snoozed_until: null, applied_at: null };
    const { data, error } = await supabase
    ...
    if (!data)
      return {
        status: "error",
        message: "Insight não encontrado ou sem permissão para alteração.",
      };
    ...
    return {
      status: "error",
      message:
        error instanceof Error ? error.message : "Falha ao atualizar insight.",
    };
  }
}
...
export async function markRecommendationRead(
  ...
  try {
    ...
    if (!data)
      return {
        status: "error",
        message: "Insight não encontrado ou sem permissão para leitura.",
      };
    revalidatePath("/insights");
    revalidatePath("/", "layout");
    ...
    return {
      ...
      message:
        error instanceof Error
          ? error.message
          : "Falha ao marcar insight como lido.",
    };
  }
}
...
export async function createKnowledgeArticle(
  ...
  const parsed = z
    .object({
      ...
      type: z.enum(["process", "manual", "checklist", "faq", "template"]),
  ...
  if (!parsed.success)
    return { status: "error", message: "Informe título, tipo e conteúdo." };
  try {
    const { workspace, supabase } = await tenantContext();
    const { data: claims } = await supabase.auth.getClaims();
    const { error } = await supabase
      ...
      .insert({
        ...
        body: { text: parsed.data.content },
        status: "draft",
        created_by: claims?.claims?.sub ?? null,
      });
    if (error) throw error;
    revalidatePath("/conhecimento");
    return { status: "success", message: "Artigo criado como rascunho." };
  } catch (error) {
    return {
      status: "error",
      message:
        error instanceof Error ? error.message : "Falha ao criar artigo.",
    };
  }
}
...
export async function updateKnowledgeArticle(
  ...
  const parsed = z
    ...
    .safeParse({ articleId, content, published });
  if (!parsed.success)
    return { status: "error", message: "Revise o conteúdo antes de salvar." };
  try {
    const { workspace, supabase } = await tenantContext();
    const { error } = await supabase
      .from("knowledge_articles")
    ...
    return {
      status: "error",
      message:
        error instanceof Error ? error.message : "Falha ao atualizar conteúdo.",
    };
  }
}
