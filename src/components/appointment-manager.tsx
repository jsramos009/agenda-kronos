...
export function AppointmentManager({ events, customers, services, demo, weekStart, today }: { events: CalendarEvent[]; customers: AppointmentOption[]; services: AppointmentOption[]; demo: boolean; weekStart: string; today: string }) {
  ...
  const todayOffset = daysBetween(weekStart, today);
  const initialDay = todayOffset >= 0 && todayOffset <= 4 ? today : weekStart;
  const [selectedSlot, setSelectedSlot] = useState(`${initialDay}T10:00`);
  const [selectedEventId, setSelectedEventId] = useState<string | null>(null);
  const [demoMessage, setDemoMessage] = useState("");
  const [notesDraft, setNotesDraft] = useState("");
  const [isUpdating, startUpdating] = useTransition();
  const [isChangingWeek, startWeekChange] = useTransition();
  ...
  const openSlot = (day: number, slot: string) => {
    setSelectedSlot(`${shiftDateKey(weekStart, day)}T${slot}`);
    setFormOpen(true);
  };
  ...
  const createDemoAppointment = (event: FormEvent<HTMLFormElement>) => {
    ...
    const day = daysBetween(weekStart, startsAt.slice(0, 10));
  ...
  const moveEvent = (appointmentId: string, day: number, slot: string) => {
    ...
    const startsAt = `${shiftDateKey(weekStart, day)}T${slot}:00-03:00`;
  ...
  const changeWeek = (distance: number) => {
    startWeekChange(() => router.push(`/agenda?week=${shiftDateKey(weekStart, distance * 7)}`));
  };
  ...
  return <>
    ...
    <div className="toolbar"><div className="date-switcher"><button aria-label="Semana anterior" disabled={isChangingWeek} onClick={() => changeWeek(-1)}><ChevronLeft size={17} /></button><strong>{isChangingWeek ? "Atualizando semana…" : weekLabel}</strong><button aria-label="Próxima semana" disabled={isChangingWeek} onClick={() => changeWeek(1)}><ChevronRight size={17} /></button></div><div><button className={`chip ${view === "week" ? "active" : ""}`} onClick={() => setView("week")}><Rows3 size={15} /> Semana</button><button className={`chip ${view === "list" ? "active" : ""}`} onClick={() => setView("list")}><List size={15} /> Lista</button><label className="chip chip--select"><Filter size={15} /><select value={serviceFilter} onChange={(event) => setServiceFilter(event.target.value)}><option value="all">Todos os serviços</option>{serviceNames.map((name) => <option key={name}>{name}</option>)}</select></label></div></div>
    ...
    {view === "week" ? <div className="calendar-scroll"><section className="calendar-panel"><div className="calendar-head"><span>GMT−3</span>{days.map((day, index) => <strong key={day} className={shiftDateKey(weekStart, index) === today ? "today" : ""}>{day}</strong>)}</div><div className="calendar-body"><div className="calendar-times">{slots.map((slot) => <span key={slot}>{slot}</span>)}</div>{days.map((day, dayIndex) => <div className="calendar-day" key={day}>{slots.map((slot) => <button onClick={() => openSlot(dayIndex, slot)} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); moveEvent(event.dataTransfer.getData("text/appointment-id"), dayIndex, slot); }} aria-label={`Criar ou mover para ${slot} em ${day}`} key={slot} />)}{filtered.filter((event) => event.day === dayIndex).map((event) => <article key={event.id} draggable onDragStart={(dragEvent) => { dragEvent.dataTransfer.setData("text/appointment-id", event.id); dragEvent.dataTransfer.effectAllowed = "move"; }} onClick={() => openEvent(event)} style={{ top: `calc(${event.start} * 68px + 4px)`, height: `calc(${event.span} * 68px - 8px)` }}><div className="calendar-event__time"><GripVertical size={13} /><span>{event.time}–{event.endTime}</span></div><strong>{event.client}</strong><small>{event.service}</small><button type="button" onClick={(clickEvent) => { clickEvent.stopPropagation(); openEvent(event); }}>Detalhes</button></article>)}</div>)}</div></section></div> : <section className="agenda-list">{filtered.length ? filtered.toSorted((a, b) => a.day - b.day || a.start - b.start).map((event) => <button key={event.id} onClick={() => openEvent(event)}><time>{days[event.day]} · {event.time}–{event.endTime}</time><div><strong>{event.client}</strong><span>{event.service}</span></div><span>Ver detalhes</span></button>) : <p className="empty-state">Nenhum agendamento nesta semana.</p>}</section>}
    {selectedEvent ? <><button className="drawer-backdrop" onClick={() => setSelectedEventId(null)} aria-label="Fechar detalhes" /><aside className="detail-drawer appointment-drawer"><button className="icon-button" onClick={() => setSelectedEventId(null)} aria-label="Fechar detalhes"><X size={18} /></button><p className="eyebrow">Agendamento</p><h2>{selectedEvent.client}</h2><p>{selectedEvent.service}</p><dl><div><dt>Data</dt><dd>{days[selectedEvent.day]}</dd></div><div><dt>Horário</dt><dd>{selectedEvent.time}–{selectedEvent.endTime}</dd></div><div><dt>Duração</dt><dd>{selectedEvent.span}h</dd></div><div><dt>Contato</dt><dd>{selectedEvent.phone || "Não informado"}</dd></div></dl><label className="field"><span>Observações</span><textarea rows={5} value={notesDraft} onChange={(event) => setNotesDraft(event.target.value)} placeholder="Inclua instruções, endereço ou observações do atendimento." /></label><button className="button button--secondary drawer-save" onClick={saveNotes} disabled={isUpdating}>{isUpdating ? "Salvando…" : "Salvar detalhes"}</button><div className="drawer-actions"><button className="button button--secondary" onClick={() => { setSelectedSlot(`${shiftDateKey(weekStart, selectedEvent.day)}T${selectedEvent.time}`); setSelectedEventId(null); setFormOpen(true); }}>Remarcar</button>{whatsappUrl ? <a className="button button--whatsapp" href={whatsappUrl} target="_blank" rel="noreferrer"><MessageCircle size={17} /> WhatsApp</a> : <button className="button button--whatsapp" disabled title="Cadastre o telefone do cliente"><MessageCircle size={17} /> WhatsApp</button>}</div><small className="drawer-hint">Você também pode arrastar o evento na grade para outro dia ou horário.</small></aside></> : null}
  </>;
}

function buildWhatsappUrl(event: CalendarEvent) {
  const phone = (event.phone ?? "").replace(/\D/g, "");
  const eventDate = new Intl.DateTimeFormat("pt-BR", { dateStyle: "long", timeZone: "America/Sao_Paulo" }).format(new Date(event.startsAt));
  const message = `Olá, ${event.client}! Seu agendamento de ${event.service} está marcado para ${eventDate}, das ${event.time} às ${event.endTime}. Se precisar ajustar o horário, responda por aqui.`;
