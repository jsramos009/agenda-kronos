...
export function CustomerManager({ customers, demo }: { customers: CustomerRow[]; demo: boolean }) {
  ...
  const createDemoCustomer = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (name.length < 2) { setDemoMessage("Informe um nome com pelo menos 2 caracteres."); return; }
    ...
    <>
      ...
      <details className="create-panel">
        ...
        <form action={demo ? undefined : action} onSubmit={createDemoCustomer} className="inline-create-form">
          <label className="field"><span>Nome</span><input name="name" required minLength={2} /></label><label className="field"><span>Telefone <small>(opcional)</small></span><input name="phone" type="tel" /></label><label className="field"><span>E-mail <small>(opcional)</small></span><input name="email" type="email" /></label><button className="button button--primary" disabled={pending}>{pending ? "Salvando…" : "Salvar cliente"}</button>
